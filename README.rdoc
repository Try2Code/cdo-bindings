...
(available in 'cdo_lib') works with ruby-1.9.x, too.
For returning multi-dimensional arrays (numpy for python,
narray for ruby) addtional netcdf-io modules are needed. These are scipy/netcdf4 for
...
The Ruby module can be used directly after loading it. For python and the ruby class interface an instance has to be created first
...
*) If you use scipy >= 0.14 as netcdf backend, you have use following code instead to avoid possible segmentation faults:
...
* 1.3 (upcomming)
   ...
   - set ruby class interface as default; module interface is still available via "require 'cdo_lib'", but will be remove in one of the next releases
...
* 1.2.7
   - Added class interface for ruby version 2.x
...
License::  Copyright 2011-2015 by Ralf Mueller
