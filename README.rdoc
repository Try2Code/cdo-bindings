...
* Return multi-dimension arrrays
   ...
   temperatures = Cdo.fldmin(:input => ifile,:returnArray => true).var('T').get     (ruby version, version < 1.2.0)
   temperatures = cdo.fldmin(input = ifile,:returnArray = True).variables['T'][:] (python version, version < 1.2.0)
   temperatures = Cdo.fldmin(:input => ifile,:returnCdf => true).var('T').get      (ruby version, version >= 1.2.0)
   temperatures = cdo.fldmin(input = ifile,:returnCdf = True).variables['T'][:]  (python version, version >= 1.2.0)
   temperatures = Cdo.fldmin(:input => ifile,:returnArray => 'T')                  (ruby version, version >= 1.2.0)
   temperatures = cdo.fldmin(input = ifile,:returnArray = 'T')                   (python version, version >= 1.2.0)
...
== Changelog
* next:
   - allow arrays in additions ot strings for input argument
